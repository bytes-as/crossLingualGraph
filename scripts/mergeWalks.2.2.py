...
def main(args):
	...
	for i, walk1 in enumerate(walks1):
		if i % 100000 == 0: print('{}/{} walks completed in first language walks'.format(i, total1))
		valid = False
		for word1 in walk1:
			...
			for word in dictionary[word1]:
				...
				for index in inverse2[word]:
					if (i, index) in written_lines: continue
					writeFile.write(' '.join(walk1) + ' ' + ' '.join(walks2[index]) + '\n')
					written_lines.add((i, index))
					written_count += 1
					if written_count % 10000000 == 0:
						print('{}/{} walks written successfully'.format(written_count, total1*total2))
		#if not valid:
